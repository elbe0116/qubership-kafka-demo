...
Apache Kafka 4.x is a significant milestone, marking the first major release to operate entirely without Apache ZooKeeper. 
By running in KRaft mode by default, Kafka simplifies deployment and management, eliminating the complexity of maintaining a separate ZooKeeper ensemble.

**Important**: It's necessary to disable kraft mode to install previous releases with Zookeeper, currently kraft mode enabled by default.
...
Health probes (liveness and readiness) are critical for Kubernetes to manage Kafka pod lifecycle. The default probe settings are tuned for typical deployments, but you may need to adjust them based on your environment:

- **Liveness Probe** checks if the Kafka process is still running. A failed liveness probe will cause Kubernetes to restart the pod.
  - Default `failureThreshold` of 20 means the pod will restart after ~5 minutes of continuous probe failures (20 * 15 seconds).
  - Increase `failureThreshold` if Kafka broker sometimes needs more time to respond during normal operations.

- **Readiness Probe** checks if the Kafka broker is ready to accept client connections. A failed readiness probe will remove the pod from service endpoints.
  - Default `failureThreshold` of 5 means the pod will be marked as not ready after ~2.5 minutes of continuous probe failures (5 * 30 seconds).
  - Adjust `initialDelaySeconds` if your Kafka startup takes longer than the default 60 seconds.

Example configuration for slower environments:

```yaml
kafka:
  livenessProbe:
    initialDelaySeconds: 120      # Wait 2 minutes before first check
    timeoutSeconds: 10             # Allow more time for probe execution
    periodSeconds: 30              # Check every 30 seconds instead of 15
    failureThreshold: 5            # Restart after 5 consecutive failures instead of 20
  readinessProbe:
    initialDelaySeconds: 90        # Wait 1.5 minutes before first check
    timeoutSeconds: 45             # Allow more time for probe execution
    periodSeconds: 60              # Check every minute instead of 30 seconds
    failureThreshold: 3            # Mark as not ready after 3 failures instead of 5
```

**Important:** Be cautious when increasing `failureThreshold` values as this delays problem detection and pod recovery.
...
| global.tls.enabled                         | boolean | no        | false         | Whether to use TLS to connect to all Kafka services. If you upgrade Kafka from non-TLS to TLS, disable `kafka.rollingUpdate` in deployment parameters.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
**Note:** The operator is installed to the same namespace as the Kafka cluster.  
To specify which namespaces the KafkaUser or AKHQ controllers should watch, use the  
`operator.kafkaUserConfigurator.watchNamespace` and  
`operator.akhqConfigurator.watchNamespace` parameters respectively.  
By default, both controllers watch all Kubernetes namespaces.
...
| operator.akhqConfigurator.watchNamespace             | string  | no        | ""                       | The comma separated list of namespaces which operator watches and processes `AkhqConfig` custom resources to organize protobuf deserialization keys auto collect. The default empty value means the controller watches all Kubernetes namespaces.                                                                             |
...
| operator.kafkaUserConfigurator.watchNamespace        | string  | no        | ""                       | The comma separated list of namespaces which operator watches and processes `KafkaUser` custom resources to organize Kafka Users declarative creating. The default empty value means the controller watches all Kubernetes namespaces.                                                                                        |
...
| kafka.externalHostNames                                | list    | no        | []                            | The broker hostnames for external access as a comma-separated list. The value can be empty. Specify the value for this parameter if you need to provide the external that is outside OpenShift/Kubernetes cluster, access for Kafka brokers.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| kafka.rollingUpdate                                    | boolean | no        | true                          | Specifies either to redeploy Kafka pods during update one by one or all in the same time. If `true` is specified, after every Kafka broker update there is a check of all brokers status.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| kafka.kraft.skipUpgradeCheck                           | boolean | no        | false                         | Whether to skip Kafka upgrade validation to 4.x versions from previous major versions like 3.x, because Kafka 4.x works only in Kraft mode and might require Zookeeper to Kraft migration procedure.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| kafka.livenessProbe.initialDelaySeconds                | integer | no        | 60                            | The initial delay in seconds before the liveness probe starts checking the container. The liveness probe verifies that Kafka broker is alive using the `./bin/kafka-health.sh live` command.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| kafka.livenessProbe.timeoutSeconds                     | integer | no        | 5                             | The timeout in seconds for each liveness probe check. If the check does not complete within this time, it is considered failed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| kafka.livenessProbe.periodSeconds                      | integer | no        | 15                            | The interval in seconds between liveness probe checks. The probe will execute every 15 seconds to verify that the broker process is alive.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| kafka.livenessProbe.successThreshold                   | integer | no        | 1                             | The number of consecutive successful liveness probe checks required to mark the container as alive after it has been detected as unhealthy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| kafka.livenessProbe.failureThreshold                   | integer | no        | 20                            | The number of consecutive failed liveness probe checks after which the container is restarted. For example, with the default value of 20 and periodSeconds of 15, the pod will restart after 20 * 15 = 300 seconds (~5 minutes) of continuous failures. For more information about liveness probes, refer to [Kubernetes Probe Documentation](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/).                                                                                                                                                                                                                                                                                                                                                                                     |
| kafka.readinessProbe.initialDelaySeconds               | integer | no        | 60                            | The initial delay in seconds before the readiness probe starts checking the container. The readiness probe verifies that Kafka broker is ready to accept client connections using the `./bin/kafka-health.sh ready` command.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| kafka.readinessProbe.timeoutSeconds                    | integer | no        | 30                            | The timeout in seconds for each readiness probe check. If the check does not complete within this time, it is considered failed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| kafka.readinessProbe.periodSeconds                     | integer | no        | 30                            | The interval in seconds between readiness probe checks. The probe will execute every 30 seconds to check if the broker is ready to serve client requests.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| kafka.readinessProbe.successThreshold                  | integer | no        | 1                             | The number of consecutive successful readiness probe checks required to mark the container as ready after it has been detected as not ready.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| kafka.readinessProbe.failureThreshold                  | integer | no        | 5                             | The number of consecutive failed readiness probe checks after which the container is marked as not ready and no traffic is sent to it. For example, with the default value of 5 and periodSeconds of 30, the pod will be marked as not ready after 5 * 30 = 150 seconds (~2.5 minutes) of continuous failures. For more information about readiness probes, refer to [Kubernetes Probe Documentation](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/).                                                                                                                                                                                                                                                                                                                             |
...
| Parameter                                            | Type    | Mandatory | Default value            | Description                                                                                                                                                                                                                                                                                                                                                                                                     |
|------------------------------------------------------|---------|-----------|--------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| monitoring.lagExporter.enabled                       | boolean | no        | false                    | Whether Kafka Exporter is to be deployed or not. The value should be equal to `true` to install the Kafka Exporter.                                                                                                                                                                                                                                                                                             |
| monitoring.lagExporter.service.image                 | string  | no        | Calculates automatically | The Docker image of Kafka Exporter.                                                                                                                                                                                                                                                                                                                                                                             |
...
| monitoring.lagExporter.clientGroupId                 | string  | no        | kafkalagexporter         | The consumer group ID of Kafka Exporter's client connections.                                                                                                                                                                                                                                                                                                                                                   |
...
| monitoring.lagExporter.kafkaLagExporterLogLevel      | string  | no        | INFO                     | The Kafka Exporter logging level.                                                                                                                                                                                                                                                                                                                                                                               |
...
| akhq.ingress.host                        | string  | no        | ""                                                                 | The name of external host which the AKHQ should be available on. It must be complex and unique enough not to intersect with other possible external hostnames. For example, to generate value for this parameter you can use the OpenShift/Kubernetes host:If URL to OpenShift/Kubernetes is ```https://search.example.com:8443``` and the namespace is `kafka-service`, the  for AKHQ can be ```akhq-kafka-service.search.example.com```. After the deployment is completed, you can access AKHQ using ```https://akhq-kafka-service.search.example.com``` URL. If this parameter is empty, Ingress is not created automatically and should be created manually to access AKHQ. |
| akhq.ingress.className                   | string  | no        | ""                                                                 | The class name of controller which the AKHQ should be available on.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| akhq.envoyGateway.enabled                | boolean | no        | false                                                              | Enables Gateway API resources for AKHQ.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| akhq.envoyGateway.host                   | string  | no        | ""                                                                 | Specifies the hostname used by the AKHQ HTTPRoute. If empty, the value from `akhq.ingress.host` will be used.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| akhq.envoyGateway.parentRefs             | list    | no        | [{"name":"default-external-gateway","namespace":"gateway-system"}] | Defines the Gateway resources that the AKHQ HTTPRoute attaches to.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| integrationTests.zookeeperHost             | string  | yes       | zookeeper                | The hostname of ZooKeeper.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| integrationTests.zookeeperPort             | integer | yes       | 2181                     | The port of ZooKeeper.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| integrationTests.kafkaHost                 | string  | yes       | kafka                    | The hostname of Kafka.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| integrationTests.kafkaPort                 | integer | yes       | 9092                     | The port of Kafka.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| integrationTests.identityProviderUrl       | string  | no        | ""                       | The URL of Identity Provider. This parameter must be specified if you want to run integration tests with `acl` tag.                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| integrationTests.backupDaemonHost          | string  | no        | kafka-backup-daemon      | The hostname of Kafka Backup Daemon.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| integrationTests.atpStorage.provider       | string  | no        | aws                      | S3 provider type for ATP Storage (for example `aws`, `minio`, `s3`). Used together with other `atpStorage` fields for S3-compatible upload of integration test reports.                                                                                                                                                                                                                                                                                                                                                              |
| integrationTests.atpStorage.serverUrl      | string  | no        | ""                       | S3 API endpoint URL.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| integrationTests.atpStorage.serverUiUrl    | string  | no        | ""                       | Optional S3 storage UI URL.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| integrationTests.atpStorage.bucket         | string  | no        | ""                       | S3 bucket name. If empty, S3 integration is typically disabled in the shared test scripts flow.                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| integrationTests.atpStorage.region         | string  | no        | us-east-1                | S3 region (for example for AWS).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| integrationTests.atpStorage.username       | string  | no        | ""                       | S3 access key. Sensitive; when report upload is enabled, values may be stored in a Kubernetes Secret.                                                                                                                                                                                                                                                                                                                                                                                                                                |
| integrationTests.atpStorage.password       | string  | no        | ""                       | S3 secret key. Sensitive; when report upload is enabled, values may be stored in a Kubernetes Secret.                                                                                                                                                                                                                                                                                                                                                                                                                                |
| integrationTests.atpReport.enabled         | boolean | no        | false                    | Opt-in for ATP report upload behavior (maps to `ATP_REPORT_ENABLED` in the integration test image flow).                                                                                                                                                                                                                                                                                                                                                                                                                             |
| integrationTests.atpReportViewUiUrl        | string  | no        | ""                       | Optional base URL for viewing Allure reports.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| integrationTests.environmentName           | string  | no        | kafka                    | Logical environment name for paths or labels in ATP Storage workflows.                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| backupDaemon.cloudBackuper.useBackupDaemon                    | boolean | no        | true                                                                          | Whether to use Kafka Backup Daemon for cloud backups. This parameter indicates if backups should be performed via the Backup Daemon service.                                                                                                                                                                                                                                                                                                                  |
| backupDaemon.cloudBackuper.backupDaemonCredentials            | boolean | no        | true                                                                          | Whether to use Backup Daemon credentials for authentication. If enabled, the Backup Daemon uses the credentials specified in `global.secrets.backupDaemon.username` and `global.secrets.backupDaemon.password` for API access.                                                                                                                                                                                                                                |
...
| cruiseControl.ingress.host              | string  | no        | ""                                                                          | The name of external host which the Cruise Control UI should be available on. It must be complex and unique enough not to intersect with other possible external hostnames.                                                                                                                                                                                                             |
| cruiseControl.envoyGateway.enabled      | boolean | no        | false                                                                       | Enables Gateway API resources for the Cruise Control UI.                                                                                                                                                                                                                                                                                                                                |
| cruiseControl.envoyGateway.host         | string  | no        | ""                                                                          | Specifies the hostname used by the Cruise Control HTTPRoute. If empty, `cruiseControl.ingress.host` will be used.                                                                                                                                                                                                                                                                       |
| cruiseControl.envoyGateway.parentRefs   | list    | no        | [{"name":"default-external-gateway","namespace":"gateway-system"}]          | Defines the Gateway resources that the Cruise Control HTTPRoute attaches to.                                                                                                                                                                                                                                                                                                            |
...
**Important**: You must migrate Kafka to Kraft mode before upgrading to 4.x versions, please refer to our migration [guide](kraft-migration.md)
...
It can be enabled with `kafka.rollingUpdate: true`, by default it is enabled. If you upgrade Kafka from non-TLS to TLS, disable `kafka.rollingUpdate` in deployment parameters.
...
1. Verify that previous `kafka-service` is installed with Helm mode.
    ...
    In that case only one option of migrations is possible.
...
* Kafka is deployed in each datacenter in HA or non-HA mode with external access.
  For more information, see [Enable Access to Kafka for External Clients](external-access.md).
